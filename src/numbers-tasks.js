...
function getAngleBetweenVectors(/* x1, y1, x2, y2 */) {
  /* const firstAngle = Math.atan2(x2, x1);
...
function getSumOfDigits(num) {
  let sumOfTheDigits = num % 10;
  let test = num;
  for (let i = String(num).length - 1; i > 0; i -= 1) {
    sumOfTheDigits += Math.floor(test / 10 ** i);
    test -= Math.floor(test / 10 ** i) * 10 ** i;
  }
